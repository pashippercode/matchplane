...
# Return a physical absolute path while allowing the final path components not
# to exist yet. Existing symlink prefixes are always resolved.
_canonicalize_path() {
    local path="$1"
    local python_spec
    local -a python_cmd=()

    if _has_control_character "$path"; then
        return 1
    fi
...
# Canonicalize a path and require it to be a strict descendant of the
# canonical allowed root. No untrusted path value is included in diagnostics.
_canonicalize_within_root() {
    local path="$1"
...
# Resolve a candidate file before it is read. The logical allowed root must
# itself remain inside the repository-owned security root, and the candidate
# must remain inside that allowed root after resolving all symlinks.
_resolve_contained_file() {
    local candidate="$1"
    local allowed_root="$2"
    local security_root="$3"
    ...
    canonical_allowed=$(_canonicalize_within_root "$allowed_root" "$security_root") || {
        echo "Error: unsafe repository file root rejected" >&2
        return 2
    }
    canonical_candidate=$(_canonicalize_within_root "$candidate" "$canonical_allowed") || {
        echo "Error: unsafe repository file rejected" >&2
        return 2
    }
    if [[ ! -f "$canonical_candidate" || ! -r "$canonical_candidate" ]]; then
        echo "Error: unsafe repository file rejected" >&2
        return 2
    fi
    printf '%s\n' "$canonical_candidate"
...
# Resolve a repository-owned write destination without requiring it to exist.
_resolve_contained_destination() {
    ...
    canonical_allowed=$(_canonicalize_within_root "$allowed_root" "$security_root") || return 1
    canonical_destination=$(_canonicalize_within_root "$destination" "$canonical_allowed") || return 1
    if [[ -e "$canonical_destination" && -d "$canonical_destination" ]]; then
        return 1
    fi
    printf '%s\n' "$canonical_destination"
}

# Canonicalize and validate the only directory that setup/check commands may
# use as feature state. Relative values are repository-relative.
validate_feature_directory() {
    local raw_path="$1"
    local repo_root="$2"
    local candidate="$raw_path"
    local canonical_feature

    if [[ -z "$raw_path" ]] || _has_control_character "$raw_path"; then
        echo "ERROR: Feature directory is invalid" >&2
        return 1
    fi
    [[ "$candidate" == /* ]] || candidate="$repo_root/$candidate"
    canonical_feature=$(_canonicalize_within_root "$candidate" "$repo_root/specs") || {
        echo "ERROR: Feature directory must be inside the repository specs directory" >&2
        return 1
    }
    if [[ -e "$canonical_feature" && ! -d "$canonical_feature" ]]; then
        echo "ERROR: Feature directory is invalid" >&2
        return 1
    fi
    printf '%s\n' "$canonical_feature"
...
read_feature_json_feature_directory() {
    ...
    if command -v jq >/dev/null 2>&1; then
        if ! _fd=$(jq -r '(.feature_directory // .feature_dir // empty) | if type == "string" and (test("[[:cntrl:]]") | not) then . else empty end' "$fj" 2>/dev/null); then
            _fd=''
        fi
    fi
    if [[ -z "$_fd" ]] && command -v python3 >/dev/null 2>&1; then
        # Use Python so pretty-printed/multi-line JSON still parses correctly.
        if ! _fd=$(python3 -c "import json,sys; d=json.load(open(sys.argv[1])); v=d.get('feature_directory', d.get('feature_dir')); print(v if isinstance(v, str) and v and not any(ord(c) < 32 or ord(c) == 127 for c in v) else '')" "$fj" 2>/dev/null); then
            _fd=''
        fi
    fi
    if [[ -z "$_fd" ]]; then
...
# Accepts only a path that has already passed validate_feature_directory().
_persist_feature_json() {
    local repo_root="$1"
    local feature_dir_value="$2"
    local fj="$repo_root/.specify/feature.json"
    fj=$(_resolve_contained_destination "$fj" "$repo_root/.specify" "$repo_root") || {
        echo "ERROR: Feature state destination is unsafe" >&2
        return 1
    }
...
get_feature_paths() {
    ...
    feature_dir=$(validate_feature_directory "$raw_feature_dir" "$repo_root") || return 1

    # Persist only the already validated canonical directory so an unsafe raw
    # override can never reach a write operation.
    ...
    if [[ -z "$current_branch" ]]; then
        local feature_dir_trimmed
        feature_dir_trimmed="${feature_dir%/}"
...
_sorted_extension_ids() {
    ...
    security_root=$(dirname "$ext_dir")
    canonical_ext_dir=$(_canonicalize_within_root "$ext_dir" "$security_root") || {
        echo "Error: unsafe extension root rejected" >&2
        return 1
    }
    if [[ ! -d "$canonical_ext_dir" ]]; then
        return 0
    fi
    ext_dir="$canonical_ext_dir"

    local registry_path="$ext_dir/.registry"
    if [[ -e "$registry_path" || -L "$registry_path" ]]; then
        local safe_registry
        safe_registry=$(_resolve_contained_file "$registry_path" "$ext_dir" "$security_root") || return 1
...
registered = {}
registry = Path(os.environ.get('SPECKIT_EXTENSION_REGISTRY', root / '.registry'))
...
" 2>/dev/null); then
            ...
            echo "Error: invalid extension registry" >&2
            return 1
        fi
    fi
...
resolve_template() {
    ...
    if _resolve_contained_file "$override" "$base/overrides" "$repo_root/.specify"; then
        return 0
    else
        candidate_status=$?
    ...
    if [ -d "$presets_dir" ]; then
        ...
        if [[ -e "$registry_file" || -L "$registry_file" ]]; then
            registry_file=$(_resolve_contained_file "$registry_file" "$presets_dir" "$repo_root/.specify") || return 2
        fi
        local python_spec=""
        local -a python_cmd=()
        if python_spec=$(_python3_command); then
        ...
        if [ -f "$registry_file" ] && [ "${#python_cmd[@]}" -gt 0 ]; then
            ...
            # The call is wrapped in an if-condition so set -e does not abort when
            # an invalid registry falls back to the directory scan below.
            ...
            if sorted_presets=$(_sorted_preset_ids "$registry_file"); then
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if _resolve_contained_file "$candidate" "$presets_dir/$preset_id" "$repo_root/.specify"; then
                            return 0
                        else
                            candidate_status=$?
                            [[ "$candidate_status" -eq 2 ]] && return 2
                ...
                for preset in "$presets_dir"/*/; do
                    ...
                    if _resolve_contained_file "$candidate" "$preset" "$repo_root/.specify"; then
                        return 0
                    else
                        candidate_status=$?
                        [[ "$candidate_status" -eq 2 ]] && return 2
    ...
    if [ -d "$ext_dir" ]; then
        ...
        while IFS= read -r extension_id; do
            ...
            if _resolve_contained_file "$candidate" "$ext" "$repo_root/.specify"; then
                return 0
            else
                candidate_status=$?
                [[ "$candidate_status" -eq 2 ]] && return 2
            fi
            candidate="$ext/${template_name}.md"
            if _resolve_contained_file "$candidate" "$ext" "$repo_root/.specify"; then
                return 0
            else
                candidate_status=$?
    ...
    if _resolve_contained_file "$core" "$base" "$repo_root/.specify"; then
        return 0
    else
        candidate_status=$?
        [[ "$candidate_status" -eq 2 ]] && return 2
...
resolve_template_content() {
    ...
    local safe_override
    local candidate_status
    if safe_override=$(_resolve_contained_file "$override" "$base/overrides" "$repo_root/.specify"); then
        if ! cat "$safe_override"; then
            echo "Error: failed to read template layer" >&2
            return 2
        fi
        return 0
    else
        ...
        if [ -n "$sorted_presets" ]; then
            while IFS= read -r preset_id; do
                ...
                if [[ -e "$manifest" || -L "$manifest" ]]; then
                    manifest=$(_resolve_contained_file "$manifest" "$presets_dir/$preset_id" "$repo_root/.specify") || return 2
...
" 2>/dev/null); then
                ...
                if [ -n "$manifest_file" ]; then
                    ...
                    if candidate=$(_resolve_contained_file "$mf" "$presets_dir/$preset_id" "$repo_root/.specify"); then
                        :
                    else
                        candidate_status=$?
                        [[ "$candidate_status" -eq 2 ]] && return 2
                ...
                if [ -z "$candidate" ] && [ "$manifest_declared" = false ]; then
                    ...
                    if candidate=$(_resolve_contained_file "$cf" "$presets_dir/$preset_id" "$repo_root/.specify"); then
                        :
                    else
                        candidate_status=$?
                        [[ "$candidate_status" -eq 2 ]] && return 2
    ...
    if [ "$effective_base_found" = false ] && [ -d "$ext_dir" ]; then
        ...
        while IFS= read -r extension_id; do
            ...
            local ext="$ext_dir/$extension_id"
            local candidate="$ext/templates/${template_name}.md"
            if candidate=$(_resolve_contained_file "$candidate" "$ext" "$repo_root/.specify"); then
                :
            else
                candidate_status=$?
                [[ "$candidate_status" -eq 2 ]] && return 2
    ...
    local core="$base/${template_name}.md"
    if [ "$effective_base_found" = false ]; then
        local safe_core
        if safe_core=$(_resolve_contained_file "$core" "$base" "$repo_root/.specify"); then
            layer_paths+=("$safe_core")
            layer_strategies+=("replace")
        else
            candidate_status=$?
            [[ "$candidate_status" -eq 2 ]] && return 2
