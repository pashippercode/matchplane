...
describe("MatchPlane workspaces", () => {
  ...
  it("keeps the root as browse plus one inline shopping conversation", async () => {
    ...
    expect(
      screen.queryByText("其他入口"),
    ).not.toBeInTheDocument();
    expect(screen.queryByText("AI 店长")).not.toBeInTheDocument();
    expect(
  ...
  it("keeps visible controls actionable instead of leaving placeholder buttons", async () => {
    ...
    const assistantToggle = screen.queryByRole("button", { name: "打开找商品" });
    if (assistantToggle) await user.click(assistantToggle);
  ...
  it("keeps theme and language controls near the shopping aid", async () => {
    ...
    expect(screen.getByRole("button", { name: "较小文字" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "较大文字" })).toBeInTheDocument();
